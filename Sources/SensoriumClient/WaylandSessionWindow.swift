...
nonisolated(unsafe) private let registryListener = heapListener(wl_registry_listener(
...
nonisolated(unsafe) private let surfaceListener = heapListener(wl_surface_listener(
    enter: { data, _, output in
        guard let window = windowFrom(data) else { return }
        let entered = output.map { ProxyPointer(pointer: $0) }
        MainActor.assumeIsolated { window.handleSurfaceEnter(output: entered?.pointer) }
    },
    leave: { _, _, _ in },
    preferred_buffer_scale: { _, _, _ in },
    preferred_buffer_transform: { _, _, _ in }
))

nonisolated(unsafe) private let outputListener = heapListener(wl_output_listener(
    geometry: { _, _, _, _, _, _, _, _, _, _ in },
    mode: { data, output, flags, _, _, refresh in
...
nonisolated(unsafe) private let frameListener = heapListener(wl_callback_listener(
...
nonisolated(unsafe) private let seatListener = heapListener(wl_seat_listener(
...
nonisolated(unsafe) private let keyboardListener = heapListener(wl_keyboard_listener(
...
nonisolated(unsafe) private let pointerListener = heapListener(wl_pointer_listener(
    enter: { data, _, serial, surface, x, y in
        guard let window = windowFrom(data) else { return }
    ...
    button: { data, _, _, _, button, buttonState in
        guard let window = windowFrom(data) else { return }
    ...
    axis_value120: { data, _, axis, value120 in
        guard let window = windowFrom(data), let scrolled = scrollAxis(axis) else { return }
        MainActor.assumeIsolated { window.handleAxisValue120(scrolled, value120: value120) }
    },
    // Which way the device itself says the wheel turned, for hardware with
    // its own inverted-scrolling setting. The value is already what the
    // person asked for by the time it arrives here.
