...
nonisolated(unsafe) private let dataDeviceListener = heapListener(wl_data_device_listener(
...
nonisolated(unsafe) private let dataOfferListener = heapListener(wl_data_offer_listener(
    offer: { data, offer, mime in
...
nonisolated(unsafe) private let dataSourceListener = heapListener(wl_data_source_listener(
    target: { _, _, _ in },
    send: { data, source, mime, fileDescriptor in
