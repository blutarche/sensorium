...
func testWaylandPasteboardTests() {
    do {
        ...
        io.currentOfferMimeTypes = ["image/png", "text/plain", "text/plain;charset=utf-8"]
        io.receiveResults = ["image/png": Data([1, 2, 3]), "text/plain;charset=utf-8": Data("A1".utf8)]
        let readout = pasteboard.read()
        expect(
            readout.loadImage() == ClipboardImage(format: .png, data: Data([1, 2, 3])),
            "an offer listing image/png first is read as its image"
        )
        expect(readout.text == nil, "and its text form is not read")
        expect(io.receivedMimes == ["image/png"], "only the preferred mime is ever fetched")

        print("PASS: an offer that lists its image first is read as the image alone")
    }

    do {
        let io = FakeWaylandDataDeviceIO()
        let pasteboard = WaylandPasteboard(io: io) { nil }
        io.currentOfferMimeTypes = ["text/plain;charset=utf-8", "image/png"]
        io.receiveResults = ["image/png": Data([1, 2, 3]), "text/plain;charset=utf-8": Data("A1".utf8)]
        let readout = pasteboard.read()
        expect(readout.text == "A1", "an offer listing a text mime first is read as its text")
        expect(readout.loadImage() == nil, "and its image form is not read")
        expect(readout.firstItemTypeIdentifiers == ["text/plain;charset=utf-8", "image/png"], "the offer's own mime order is what the engine chooses by")
        expect(io.receivedMimes == ["text/plain;charset=utf-8"], "only the preferred mime is ever fetched")

        print("PASS: an offer that lists its text first is read as the text alone")
    }
    ...
    do {
        ...
        io.changeCount = 1
        expect(
            engine.poll() == .send(.image(format: .png, data: Data([1, 2, 3]))),
            "a preferred text form over the limit falls back to the image"
        )
        expect(io.receivedMimes == ["text/plain", "image/png"], "which is read only once the text was too large")

        print("PASS: an oversize preferred form falls back to the other form on the same offer")
    }
    ...
    do {
        ...
        io.changeCount = 1
        expect(engine.poll() == .nothingToSend, "a read that timed out is not reported as an unsupported copy")

        print("PASS: a failed read is nothing to send rather than a refusal")
    }
    ...
    do {
        ...
        let readout = pasteboard.read()
        expect(readout.text == "hello", "the utf-8 charset flavour is preferred over the bare plain one")
    ...
    do {
        ...
        expect(readout.text == nil && readout.loadImage() == nil, "and carries no content")
    ...
    do {
        ...
        let readout = pasteboard.read()
        expect(readout.text == "mine", "an own-source offer returns what this pasteboard last wrote")
    ...
    do {
        ...
        expect(first.text == "same", "the first read of a genuine offer returns its content")
        // A focus round trip re-announces the very same selection.
        io.currentOfferMimeTypes = ["text/plain"]
        let second = pasteboard.read()
        expect(!second.hasItems, "a re-offer of the same content is suppressed as a duplicate, not reported as unsupported")
    ...
    do {
        ...
        io.currentOfferMimeTypes = ["text/plain", "x-kde-passwordManagerHint"]
        let first = pasteboard.read()
        expect(first.hasItems && first.isExcludedByType, "the first read of an excluded offer is reported as excluded")
        io.currentOfferMimeTypes = ["text/plain", "x-kde-passwordManagerHint"]
        let second = pasteboard.read()
        expect(!second.hasItems, "a re-offer of the same excluded offer is nothing new")
        expect(!second.isExcludedByType, "and is not reported as excluded again")

        print("PASS: a focus round trip re-offering an excluded offer reads as nothing new")
    }

    do {
        let io = FakeWaylandDataDeviceIO()
        let pasteboard = WaylandPasteboard(io: io) { nil }
        io.currentOfferMimeTypes = ["text/uri-list"]
        let first = pasteboard.read()
        expect(first.hasItems && !first.isExcludedByType, "the first read of an unsupported offer is reported as unsupported")
        io.currentOfferMimeTypes = ["text/uri-list"]
        let second = pasteboard.read()
        expect(!second.hasItems, "a re-offer of the same unsupported offer is nothing new")

        print("PASS: a focus round trip re-offering an unsupported offer reads as nothing new")
    }
    ...
    do {
        ...
        _ = pasteboard.read()
        io.currentOfferMimeTypes = ["text/plain"]
        io.receiveResults = ["text/plain": Data("between".utf8)]
        let between = pasteboard.read()
        expect(between.text == "between", "a different copy in between is read normally")
        io.currentOfferMimeTypes = ["text/plain", "x-kde-passwordManagerHint"]
        let again = pasteboard.read()
        expect(again.hasItems && again.isExcludedByType, "the same excluded offer after a different copy is reported again")

        print("PASS: an excluded offer copied again after a different copy is reported again")
    }
    ...
    do {
        ...
        expect(readout.text == "someone else's", "and returns whatever that read produced")
    ...
    do {
        ...
        expect(readout.text == "someone else's", "and returns that external content")
    ...
    do {
        ...
        let readout = pasteboard.read()
        expect(readout.text == "fallback", "a preferred mime with invalid UTF-8 bytes falls through to the next mime rather than losing the read")
    ...
    do {
        ...
        expect(!afterClear.hasItems, "clear() drops the last written content, so an own-source read after it has nothing to return")
    ...
    do {
        ...
        pasteboard.clear()
        let second = pasteboard.read()
        expect(second.text == "same", "clear() drops the duplicate-suppression state, so a re-offer of the same bytes reads as fresh again")
