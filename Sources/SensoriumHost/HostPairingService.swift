...
public final class HostPairingService {
    ...
    /// Un-pairs `publicKey` for this running host and in its store together,
    /// so the key a later authenticated hello is checked against and the
    /// list the host window reads can never disagree.
    public func removeApprovedDevice(_ publicKey: Data) {
        approvedKeys.remove(publicKey)
        approvedStore?.remove(publicKey)
    }
