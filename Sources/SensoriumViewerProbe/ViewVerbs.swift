...
enum ViewVerbs {
    ...
    ///
    /// `sharesClipboard` is the macOS viewer's own default: off. A clipboard
    /// session is built either way, so a tag 3 packet always reaches the
    /// real apply path rather than being silently dropped by a nil one; the
    /// flag only decides whether this machine's own copies are ever polled
    /// and sent.
    @MainActor
