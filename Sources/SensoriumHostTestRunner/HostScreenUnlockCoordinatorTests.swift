...
func runHostScreenLiveSessionTests() async {
    ...
    do {
        ...
        let claimA = registry.admit(devicePublicKey: deviceA, isLive: { aLive.isLive }, stop: {})
        expect(claimA != nil, "a first host-screen session for a device is admitted")
        expect(registry.admit(devicePublicKey: deviceA, isLive: { true }, stop: {}) == nil, "a second concurrent session for the same device is refused")
        expect(registry.admit(devicePublicKey: deviceB, isLive: { true }, stop: {}) != nil, "a different device is unaffected and admits")
        ...
        aLive.set(false)
        let claimAAgain = registry.admit(devicePublicKey: deviceA, isLive: { true }, stop: {})
        ...
        expect(registry.admit(devicePublicKey: deviceA, isLive: { true }, stop: {}) == nil, "releasing a stale claim never evicts the newer live session that replaced it")
...
private final class ReleaseCountingRegistry: HostScreenLiveSessionRegistering {
    ...
    func admit(
        ...
        inner.admit(devicePublicKey: devicePublicKey, isLive: isLive, stop: stop)
    }
    ...
    func stopSession(for devicePublicKey: Data) {
        inner.stopSession(for: devicePublicKey)
    }
}
