...
- **While the screen is locked**, every event -- key, pointer, and scroll alike -- posts at `.cghidEventTap` instead of `.cgSessionEventTap`, the same tap a system hotkey posts to.
...
An opt-in action inside a live, authenticated host-screen session: the person at the viewer types the host's own login password, and the host types it into its own locked login window.

The typed password is not the only input reaching that window: an ordinary click or scroll in the session canvas reaches the real lock screen exactly as a key does, through the same locked-screen routing (see §4), whether or not this unlock flow is ever used.

### 9.1 Gating

Lock-screen unlock is reachable only from a host-screen session that is already live and already authenticated. It adds nothing to the session's own admission: it opens no separate connection and asks for no separate arming record. A session canvas has no login window and offers no unlock. Out of scope entirely: FileVault's pre-boot, cold-boot unlock screen. The built-in screen-sharing service this relies on is not running there, so a host is only ever reachable through this path once it is already past FileVault, sitting at its ordinary login window.

### 9.2 The flow

1. The viewer sends `hostScreenUnlockRequest(password)`, the password as raw UTF-8 bytes.
2. The host answers `hostScreenUnlockResult(outcome)`, then `hostScreenLockState(locked)` so the viewer knows whether to keep offering the prompt.

The host also rereads its own lock state every two seconds for as long as the host-screen session lasts, and sends another `hostScreenLockState` whenever that reading changes -- not only right after an unlock attempt. This is what lets the viewer offer the prompt when the person at the host locks the screen mid-session, and drop it if they unlock the screen themselves at the machine.

See `docs/protocol.md` for the wire messages and outcome tokens in full.
