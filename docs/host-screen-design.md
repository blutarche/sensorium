...
- **Local activity** is read with `CGEventSource.secondsSinceLastEventType` against `.hidSystemState`. Input the host posts can touch this. A key event posted at `.cgSessionEventTap` reset it on a real host, as a post at `.cghidEventTap` does. So every post, at either tap, is recorded as the host's own, and that record is discounted for the relock decision (§9.5). Relock must not stay open just because a viewer is typing. Without the discount, a viewer's own forwarded typing would read as a person at the machine, and relock would never fire while a session is live. The ask-first presence gate (§6.2) reads the same counter raw instead: over-reporting activity there only means asking more often, the safe direction, while discounting it could hide a real person at the machine from a prompt meant to ask them. A system hotkey such as Mission Control, Spaces, or Spotlight posts to `.cghidEventTap`, early enough for the window server's own handling to see it. It carries `CoreGraphicsInputTranslation.nativeAuxiliaryFlags` so navigation-cluster and F-row keys read correctly. Every event posted while the screen is locked goes there too, below.
...
- **While a session is live.** One prevent-sleep assertion, taken once however many surfaces the session has. A prevent-sleep assertion keeps a display that is already awake from sleeping; it does not wake one that already has. So a live host-screen session also re-declares user activity on its own, on an interval well under macOS's shortest display-off timeout, for as long as it is streaming, and stops the moment it is not. A session canvas already reads the whole machine rather than one display it alone answers for, so it has no equivalent need and keeps declaring only at the two points above.
...
A host-screen capture can also stop on its own: ScreenCaptureKit's own signal that its stream has ended, distinct from the tick-driven silence above, which reads what a stream delivers rather than whether the stream itself is still there. This happens when the display it was capturing goes offline or to sleep mid-session, or when a hardware mirror set's "primary" role moves onto a sibling display without any monitor actually going anywhere. The session survives: the badge naming the connected device stays up, the session record stays open, and the host wakes its displays and waits for this session's own target -- found by the exact display this session was admitted for and confirmed against its identity, never a different display of the same model -- to read online, awake, and not now a mirror member, then rebuilds capture on it and sends the viewer a key frame. This is retried, waking the displays again each time, for up to about thirty seconds. A display that has become a mirror member is never captured in its place, and ends the session at once rather than waiting out that bound for an answer that will not change.

If the session still ends -- because the display never came back, or came back only as a mirror member -- it ends with `host-displays-asleep`, and the viewer is told the host's screens were asleep. The viewer does not redial into that ending: a person waking the screen, or turning off mirroring, is what changes the answer. `capture-unavailable` names the different case where the host can no longer get a picture out of that machine at all, and tells a person to quit and reopen the host.
