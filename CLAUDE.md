...
Sensorium ships as two double-clickable macOS apps and nothing else. The host app may carry a privileged helper inside its own bundle, installed only through the operating system's own approval flow. No user ever runs a command, opens Terminal, or types an address. The CLI verbs and the `.command` launchers are test scaffolding — not an alpha surface — and every capability must be reachable from the GUI before it counts as delivered.
...
- Do NOT install packages, Xcode, Homebrew formulae, applications, services, agents, launch daemons, login items, or permissions on a development machine. Shipping code may register the host's own bundled helper through `SMAppService`; installing a build of it on the owner's machines happens only when the owner asks.
...
A locked host is unlocked the way it would be at the machine: the person at
the viewer sees the real lock screen in the picture and types into it. The
viewer draws no prompt, panel, or notice of its own, and keys are forwarded
exactly as when the host is unlocked. The host delivers them to the lock
screen through its bundled privileged helper, one keystroke at a time, so
Sensorium never assembles, stores, or logs a password. FileVault's
pre-boot unlock is out of scope.
